...
func HandleUserPromptSubmit(paths Paths, payload UserPromptPayload, now time.Time) (HookResult, error) {
	...
	if prompt == "" || !LooksLikeActivation(prompt) {
		return nil, nil
	}
	if now.IsZero() {
		now = time.Now().UTC()
	...
	if err != nil {
		return BlockWithReason(fmt.Sprintf("Claude Code loop activation failed: %v", err)), nil
	}
	if !ok {
		return nil, nil
	}
	if strings.TrimSpace(payload.SessionID) == "" {
		return BlockWithReason("Claude Code loop activation failed: cc-loop activation requires a session_id from Claude Code"), nil
	}
...
func (p UserPromptPayload) PromptText() string {
...
func HandleStop(ctx context.Context, paths Paths, payload StopPayload, now time.Time) (HookResult, error) {
	...
	if err != nil {
		return StopWarning(fmt.Sprintf("Claude Code loop stop hook failed: %v", err)), nil
	}
	if active == nil {
		return nil, nil
	}
	...
	if limitMode == LimitModeTime {
		...
		if deadlineAt == nil {
			return StopWarning("Claude Code loop stop hook failed: active cc-loop is missing deadline_at"), nil
		}
		...
		if targetRounds <= 0 {
			return StopWarning("Claude Code loop stop hook failed: active cc-loop is missing a positive target_rounds"), nil
		}
		...
		if checkResult.Outcome == GoalCheckOutcomeCompleted {
			...
			if err := AppendGoalCheckLog(paths, active.Path, record, checkResult, false, false, now); err != nil {
				return StopWarning(fmt.Sprintf("Claude Code loop goal log failed: %v", err)), nil
			}
			return nil, nil
		}
	} else {
		return StopWarning(fmt.Sprintf("Claude Code loop stop hook failed: unsupported limit mode %q", limitMode)), nil
	}
	...
	if rapidCount >= RapidStopLimit && record.EscalationUsed {
		...
		if goalResult != nil {
			if err := AppendGoalCheckLog(paths, active.Path, record, *goalResult, false, false, now); err != nil {
				return StopWarning(fmt.Sprintf("Claude Code loop stopped after repeated rapid completions, but goal logging failed: %v", err)), nil
			}
		}
		return StopWarning("Claude Code loop stopped after repeated rapid completions. Review the latest result manually before reactivating the loop."), nil
	}
...
func GoalContinuationReason(runtimeConfig RuntimeConfig, record LoopRecord, result goalCheckResult, aggressive bool) string {
	...
	lines := []string{
		"Continue the active cc-loop goal task.",
...
func ContinuationReason(runtimeConfig RuntimeConfig, record LoopRecord, remainingSeconds *int, aggressive bool) string {
	...
	lines := []string{"Continue the active cc-loop task."}
