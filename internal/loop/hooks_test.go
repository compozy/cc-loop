...
func TestUserPromptSubmitCreatesTimeLoopFile(t *testing.T) {
	...
	result, err := HandleUserPromptSubmit(paths, UserPromptPayload{
		SessionID: "sess-1",
		CWD:       repoRoot,
		Prompt:    `[[CC_LOOP name="release-stress-qa" min="6h"]]` + "\nRun the QA task.",
	}, now)
	if err != nil {
...
func TestUserPromptSubmitSupersedesPreviousLoopForSession(t *testing.T) {
	...
	_, err := HandleUserPromptSubmit(paths, UserPromptPayload{
		SessionID: "sess-1",
		CWD:       repoRoot,
		Prompt:    `[[CC_LOOP name="first-run" min="1h"]]` + "\nFirst task.",
	}, start)
	if err != nil {
	...
	_, err = HandleUserPromptSubmit(paths, UserPromptPayload{
		SessionID: "sess-1",
		CWD:       repoRoot,
		Prompt:    `[[CC_LOOP name="second-run" rounds="2"]]` + "\nSecond task.",
...
func TestUserPromptSubmitCreatesGoalLoopFile(t *testing.T) {
	...
	result, err := HandleUserPromptSubmit(paths, UserPromptPayload{
		SessionID: "sess-1",
		CWD:       repoRoot,
		Prompt:    `[[CC_LOOP name="goal-qa" goal="verify release" confirm_model="opus" confirm_reasoning_effort="high"]]` + "\nRun the QA task.",
	}, fixedTime())
	if err != nil {
		t.Fatalf("handle user prompt submit: %v", err)
	}
	...
	if record.ConfirmModel == nil || *record.ConfirmModel != "opus" {
...
func TestStopGoalModeCompletesWhenConfirmed(t *testing.T) {
	...
	installFakeInterpreterClaude(t)
...
`)
	...
	start := fixedTime()
	path := writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="goal-qa" goal="verify release"]]`+"\nRun the QA task.", start)
	latest := "Done."
	...
	assertContains(t, readText(t, argsPath), "--effort\nhigh")
	...
	assertContains(t, readText(t, interpreterArgsPath), "--json-schema")
	...
	assertContains(t, readText(t, paths.RunsLogPath()), `"interpret_model":"haiku"`)
}
...
func TestStopGoalModeContinuesWhenIncompleteAndRunsPreLoopContinue(t *testing.T) {
	...
	fakeClaude := writeFakeClaude(t)
	installFakeInterpreterClaude(t)
	t.Setenv("FAKE_CLAUDE_ARGS", filepath.Join(t.TempDir(), "args.txt"))
	t.Setenv("FAKE_CLAUDE_STDIN", filepath.Join(t.TempDir(), "stdin.txt"))
	t.Setenv("FAKE_CLAUDE_REVIEW", "The task is not complete. Missing work: run integration tests.")
...
func TestStopGoalModeContinuesOnConfirmationFailure(t *testing.T) {
	...
	t.Setenv("FAKE_CLAUDE_REVIEW", "not used")
	writeRuntimeConfig(t, paths, `[goal]
`+fakeGoalConfirmCommandConfig(fakeClaude)+`
timeout_seconds = 5
`)
	...
	start := fixedTime()
	writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="goal-qa" goal="verify release"]]`+"\nRun the QA task.", start)
...
func TestStopGoalModeUsesPayloadModelWhenConfigBlankAndOmitsBlankReasoning(t *testing.T) {
	...
	t.Setenv("FAKE_CLAUDE_ARGS", argsPath)
	t.Setenv("FAKE_CLAUDE_STDIN", filepath.Join(t.TempDir(), "stdin.txt"))
	t.Setenv("FAKE_CLAUDE_REVIEW", "The task needs more evidence before completion.")
...
func TestStopGoalModeTimesOutConfirmation(t *testing.T) {
	...
	t.Setenv("FAKE_CLAUDE_REVIEW", "late")
	writeRuntimeConfig(t, paths, `[goal]
`+fakeGoalConfirmCommandConfig(fakeClaude)+`
timeout_seconds = 1
`)
...
func TestStopGoalModeContinuesOnInterpretationInvalidOutput(t *testing.T) {
	...
	t.Setenv("FAKE_CLAUDE_REVIEW", review)
...
func TestStopContinuesBeforeDeadlineWithOptionalGuidance(t *testing.T) {
	...
	paths := mustPaths(t)
	repoRoot := filepath.Join(t.TempDir(), "repo")
	skillDir := filepath.Join(repoRoot, ".claude", "skills", "focused-qa")
...
optional_skill_path = ".claude/skills/focused-qa"
extra_continuation_guidance = "Capture concrete evidence before you stop."
`)

	start := fixedTime()
	path := writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="release-stress-qa" min="6h"]]`+"\nRun the QA task.", start)
...
func TestStopMarksTimeLoopCompletedAfterDeadline(t *testing.T) {
	...
	start := fixedTime()
	path := writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="release-stress-qa" min="5m"]]`+"\nRun the QA task.", start)
	latest := "Done."
...
func TestStopRoundsModeCompletesTargetRounds(t *testing.T) {
	...
	start := fixedTime()
	path := writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="release-stress-qa" rounds="3"]]`+"\nRun the QA task.", start)
...
func TestStopEscalatesOnceThenCutsShortInRoundsMode(t *testing.T) {
	...
	start := fixedTime()
	path := writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="release-stress-qa" rounds="5"]]`+"\nRun the QA task.", start)
...
`)
	...
	start := fixedTime()
	writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="release-stress-qa" rounds="2"]]`+"\nRun the QA task.", start)
...
`)
	...
	start := fixedTime()
	writeLoop(t, paths, "sess-1", repoRoot, `[[CC_LOOP name="release-stress-qa" rounds="1"]]`+"\nRun the QA task.", start)
	result, err := HandleStop(context.Background(), paths, StopPayload{
		SessionID: "sess-1",
		CWD:       repoRoot,
	}, start.Add(time.Minute))
...
func mustPaths(t *testing.T) Paths {
	t.Helper()
	paths, err := NewPaths(filepath.Join(t.TempDir(), ".claude"))
	if err != nil {
...
func fakeGoalConfirmCommandConfig(fakeClaude string) string {
	return `confirm_command = "` + fakeClaude + ` -p --output-format text $MODEL_ARGV $REASONING_ARGV"`
}
...
func installFakeInterpreterClaude(t *testing.T) {
	t.Helper()
	dir := t.TempDir()
...
printf '%s\n' "${FAKE_INTERPRET_VERDICT:-}"
exit "${FAKE_INTERPRET_EXIT:-0}"
`
	if err := os.WriteFile(path, []byte(script), 0o755); err != nil {
		t.Fatalf("write fake interpreter claude: %v", err)
	}
