...
confirm_command = "bin/claude exec $MODEL_ARGV $REASONING_ARGV $PROMPT_FILE"
timeout_seconds = 90
...
`)
	...
	if cfg.Goal.ConfirmCommand != "bin/claude exec $MODEL_ARGV $REASONING_ARGV $PROMPT_FILE" {
	...
	if cfg.Goal.InterpretModel != "haiku" {
...
func TestRuntimeConfigDefaultGoalConfirmCommandUsesClaudePrint(t *testing.T) {
	t.Parallel()

	cfg := LoadRuntimeConfig(mustPaths(t))

	if !strings.Contains(cfg.Goal.ConfirmCommand, "claude -p") {
		t.Fatalf("expected default confirm command to use claude print mode, got %#v", cfg.Goal.ConfirmCommand)
	}
	if !strings.Contains(cfg.Goal.ConfirmCommand, "--permission-mode bypassPermissions") {
		t.Fatalf("expected default confirm command to use bypassPermissions, got %#v", cfg.Goal.ConfirmCommand)
	}
	if !strings.Contains(cfg.Goal.ConfirmCommand, "--output-format text") {
		t.Fatalf("expected default confirm command to request text output, got %#v", cfg.Goal.ConfirmCommand)
	}
	...
	interpretCommand := defaultGoalInterpreterClaudePrint()
	if !strings.Contains(interpretCommand, "--json-schema") {
		t.Fatalf("expected fixed interpreter command to include structured output, got %#v", interpretCommand)
	}
	if !strings.Contains(interpretCommand, "$INTERPRET_SCHEMA") {
		t.Fatalf("expected fixed interpreter command to include interpreter schema placeholder, got %#v", interpretCommand)
	}
	if !strings.Contains(interpretCommand, "--output-format json") {
		t.Fatalf("expected fixed interpreter command to request JSON output, got %#v", interpretCommand)
	}
}
