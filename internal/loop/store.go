...
func DefaultPaths() (Paths, error) {
	...
	return NewPaths(filepath.Join(home, ".claude"))
}

func NewPaths(claudeConfigDir string) (Paths, error) {
	if claudeConfigDir == "" {
		return Paths{}, fmt.Errorf("Claude Code config directory is required")
	}
	expanded := claudeConfigDir
	if expanded == "~" {
		home, err := os.UserHomeDir()
		if err != nil {
			return Paths{}, fmt.Errorf("resolve user home: %w", err)
	...
	if err != nil {
		return Paths{}, fmt.Errorf("resolve Claude Code config directory %q: %w", claudeConfigDir, err)
	}
	return Paths{ClaudeConfigDir: abs}, nil
}
...
func (p Paths) RuntimeRoot() string {
	return filepath.Join(p.ClaudeConfigDir, RuntimeName)
}
...
func ResolveWorkspaceRoot(start string) (string, error) {
	...
	for {
		if info, statErr := os.Stat(filepath.Join(current, ".claude")); statErr == nil && info.IsDir() {
			return current, nil
		}
